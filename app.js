...
const showSmsButton = (body) => {
    ...
    if (c1) {
        buttonsHTML += `<a href="${smsUrlBuilder(c1, body)}" style="background: #CC0000; display:block; padding: 20px; color: white; border-radius: 12px; text-decoration: none; font-weight: bold; text-align: center; margin-bottom:10px; border-bottom: 4px solid #8B0000;">🚨 ALERT TOPFORD CSO</a>`;
    }
    if (c2) {
        buttonsHTML += `<a href="${smsUrlBuilder(c2, body)}" style="background: #003366; display:block; padding: 20px; color: white; border-radius: 12px; text-decoration: none; font-weight: bold; text-align: center; border-bottom: 4px solid #001a33;">🛡️ TOPFORD INT. DESK</a>`;
    }
    if (!c1 && !c2) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const cancelSOS = () => {
        ...
        statusMsg.innerText = "TOPFORD-SAFE-SIGNAL Ready";
    };
    ...
    const finishSOS = () => {
        ...
        const studentId = localStorage.getItem('vgn_allergies') || "M.B. ISHAKA";
        ...
        navigator.geolocation.getCurrentPosition((pos) => {
            ...
            const smsBody = `🚨 TOPFORD-SAFE-SIGNAL!\nLOCATION: ${hostel}\nCCSO-ID: ${studentId}\nGPS: ${mapUrl}\nSTATUS: URGENT.`;
